...
void GuiSystem::render(Renderer& renderer, const Camera& camera, float deltaTime, float fps) {
    ...
    if (ImGui::Begin("Engine Controls", &visible, windowFlags)) {
        ...
        if (ImGui::BeginTabBar("ControlTabs")) {
            ...
            if (ImGui::BeginTabItem("Tree Editor")) {
                renderTreeEditorSection(renderer);
                ImGui::EndTabItem();
            }
            ImGui::EndTabBar();
        }
...
void GuiSystem::renderTreeEditorSection(Renderer& renderer) {
    ImGui::Spacing();
    ...
    if (ImGui::SliderFloat("Growth Influence", &params.growthInfluence, 0.0f, 1.0f)) changed = true;
