...
void AtmosphereLUTSystem::destroyLUTResources() {
    ...
    if (rayleighIrradianceLUT != VK_NULL_HANDLE) {
        vmaDestroyImage(allocator, rayleighIrradianceLUT, rayleighIrradianceAllocation);
    ...
    if (mieIrradianceLUT != VK_NULL_HANDLE) {
        vmaDestroyImage(allocator, mieIrradianceLUT, mieIrradianceAllocation);
...
bool AtmosphereLUTSystem::createIrradianceLUTs() {
    auto createIrradianceImage = [&](VkImage& image, VmaAllocation& allocation, VkImageView& view) -> bool {
        VkImageCreateInfo imageInfo{};
        imageInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
        imageInfo.imageType = VK_IMAGE_TYPE_2D;
        imageInfo.format = VK_FORMAT_R16G16B16A16_SFLOAT;
        ...
        if (vmaCreateImage(allocator, &imageInfo, &allocInfo, &image, &allocation, nullptr) != VK_SUCCESS) {
            SDL_Log("Failed to create irradiance LUT image");
            return false;
        }
        ...
        if (vkCreateImageView(device, &viewInfo, nullptr, &view) != VK_SUCCESS) {
            SDL_Log("Failed to create irradiance LUT view");
            return false;
        }
    ...
    if (!createIrradianceImage(rayleighIrradianceLUT, rayleighIrradianceAllocation, rayleighIrradianceLUTView)) {
        return false;
    }
    if (!createIrradianceImage(mieIrradianceLUT, mieIrradianceAllocation, mieIrradianceLUTView)) {
        return false;
    }
...
bool AtmosphereLUTSystem::createDescriptorSetLayouts() {
    ...
    // Irradiance LUT descriptor set layout (Rayleigh + Mie outputs, transmittance input, uniform buffer)
    {
        std::array<VkDescriptorSetLayoutBinding, 4> bindings{};

        // Rayleigh irradiance output
        bindings[0].binding = 0;
        bindings[0].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
        bindings[0].descriptorCount = 1;
        bindings[0].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;

        // Mie irradiance output
        bindings[1].binding = 1;
...
bool AtmosphereLUTSystem::createDescriptorSets() {
    ...
    // Allocate irradiance descriptor set
    {
        ...
        rayleighInfo.imageView = rayleighIrradianceLUTView;
        rayleighInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
        ...
        writes[0].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
        writes[0].descriptorCount = 1;
        writes[0].pImageInfo = &rayleighInfo;

        VkDescriptorImageInfo mieInfo{};
        mieInfo.imageView = mieIrradianceLUTView;
        mieInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
        ...
        writes[2].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        writes[2].descriptorCount = 1;
        writes[2].pImageInfo = &transmittanceInfo;
...
void AtmosphereLUTSystem::computeSkyViewLUT(VkCommandBuffer cmd, const glm::vec3& sunDir,
    ...
    VkImageMemoryBarrier barrier{};
    barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
    barrier.oldLayout = skyViewInitialized ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_UNDEFINED;
    barrier.newLayout = VK_IMAGE_LAYOUT_GENERAL;
    barrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    barrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    ...
    barrier.subresourceRange.baseArrayLayer = 0;
    barrier.subresourceRange.layerCount = 1;
    barrier.srcAccessMask = skyViewInitialized ? VK_ACCESS_SHADER_READ_BIT : 0;
    barrier.dstAccessMask = VK_ACCESS_SHADER_WRITE_BIT;

    vkCmdPipelineBarrier(cmd,
                         skyViewInitialized ? VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT | VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT
    ...
    skyViewInitialized = true;
    SDL_Log("Computed sky-view LUT (%dx%d)", SKYVIEW_WIDTH, SKYVIEW_HEIGHT);
}

void AtmosphereLUTSystem::computeIrradianceLUT(VkCommandBuffer cmd, const glm::vec3& sunDir) {
    AtmosphereLUTUniforms uniforms{};
    uniforms.params = atmosphereParams;
    uniforms.sunDirection = glm::vec4(sunDir, 0.0f);
    memcpy(uniformMappedPtr, &uniforms, sizeof(AtmosphereLUTUniforms));
    ...
    VkImageMemoryBarrier barrier{};
    barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
    barrier.oldLayout = irradianceInitialized ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_UNDEFINED;
    barrier.newLayout = VK_IMAGE_LAYOUT_GENERAL;
    barrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    barrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    barrier.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
    ...
    barrier.subresourceRange.baseArrayLayer = 0;
    barrier.subresourceRange.layerCount = 1;
    barrier.srcAccessMask = irradianceInitialized ? VK_ACCESS_SHADER_READ_BIT : 0;
    barrier.dstAccessMask = VK_ACCESS_SHADER_WRITE_BIT;

    barrier.image = rayleighIrradianceLUT;
    vkCmdPipelineBarrier(cmd,
                         irradianceInitialized ? VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT
                                              : VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                         VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                         0, 0, nullptr, 0, nullptr, 1, &barrier);

    barrier.image = mieIrradianceLUT;
    vkCmdPipelineBarrier(cmd,
                         irradianceInitialized ? VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT
                                              : VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                         VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
...
bool AtmosphereLUTSystem::exportLUTsAsPNG(const std::string& outputDir) {
    ...
    success &= exportImageToPNG(rayleighIrradianceLUT, VK_FORMAT_R16G16B16A16_SFLOAT,
                                IRRADIANCE_WIDTH, IRRADIANCE_HEIGHT,
                                outputDir + "/irradiance_rayleigh_lut.png");

    success &= exportImageToPNG(mieIrradianceLUT, VK_FORMAT_R16G16B16A16_SFLOAT,
                                IRRADIANCE_WIDTH, IRRADIANCE_HEIGHT,
                                outputDir + "/irradiance_mie_lut.png");
