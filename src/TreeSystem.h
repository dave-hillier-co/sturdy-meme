...
// Leaf instance - generated by compute shader for canopy
struct LeafInstance {
    glm::vec4 position;         // xyz = world position, w = size
    glm::vec4 facing;           // xy = facing direction, z = hash, w = flutter
    glm::vec4 color;            // rgb = color tint, a = alpha
    glm::uvec4 metadata;        // x = tree index, y = flags, z = unused, w = unused
};
...
public:
    ...
    void recordLeafDraw(VkCommandBuffer cmd, uint32_t frameIndex, float time);
...
private:
    ...
    bool createLeafComputePipeline();
    ...
    // Leaf compute pipeline
    VkDescriptorSetLayout leafComputeDescriptorSetLayout = VK_NULL_HANDLE;
    VkPipelineLayout leafComputePipelineLayout = VK_NULL_HANDLE;
    VkPipeline leafComputePipeline = VK_NULL_HANDLE;

    // Leaf graphics pipeline
    VkDescriptorSetLayout leafGraphicsDescriptorSetLayout = VK_NULL_HANDLE;
    ...
    // Double-buffered storage buffers for generated leaves
    BufferUtils::DoubleBufferedBufferSet leafBuffers;
    BufferUtils::DoubleBufferedBufferSet leafIndirectBuffers;
    ...
    static constexpr uint32_t MAX_LEAVES = 100000;   // Generated leaves per frame
    ...
    // Each leaf is a billboard quad: 2 triangles * 3 verts = 6 vertices
    static constexpr uint32_t VERTICES_PER_LEAF = 6;
