...
void TreeRenderer::render(vk::CommandBuffer cmd, uint32_t frameIndex, float time,
    ...
    // Render branches by iterating the ordered, baked renderable list.
    // Position i in branchRenderables == treeInstanceIndex == lodStates_[i] ==
    // GPU slot, so reading transform/bark/mesh from the renderable is identical
    // to the old per-frame ECS view but keeps order decoupled from entt.
...
void TreeRenderer::renderShadows(vk::CommandBuffer cmd, uint32_t frameIndex,
    ...
    if (renderBranches && !branchRenderables.empty()) {
        ...
        if (useInstancedPath) {
            ...
            // Iterate the ordered baked renderable list (position i ==
            // treeInstanceIndex == lodStates_[i]); decoupled from the entt view.
            for (const auto& renderable : branchRenderables) {
                int treeIdx = renderable.treeInstanceIndex;
                uint32_t lodIndex = (treeIdx >= 0) ? static_cast<uint32_t>(treeIdx) : 0;
