...
void GameMenu::renderSettings() {
    ...
    IPerformanceControl* perf =
        hooks_.performanceControl ? hooks_.performanceControl() : nullptr;
    if (perf) {
        auto toggles = perf->getPerformanceToggles().getAllToggles();
        for (const QualityEntry& entry : kQualityEntries) {
            for (const auto& toggle : toggles) {
                if (toggle.name != entry.toggleName) continue;
                if (ImGui::Checkbox(entry.label, toggle.value)) {
                    perf->syncPerformanceToggles();
                }
                break;
            }
        }
